...
SERVER=$(jq -r '.data.myself.pods[] | select ( any(.; .runtime != null)).runtime.ports[] | select ( any(.; .privatePort == 22)).ip' < /tmp/rp)
PORT=$(jq -r '.data.myself.pods[] | select ( any(.; .runtime != null)).runtime.ports[] | select ( any(.; .privatePort == 22)).publicPort' < /tmp/rp)
